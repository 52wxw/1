...
const login = async () => {
  ...
  // 简单验证
  if (!form.username) {
    errors.username = t('pages.login.usernameRequired'); // 修改为 t()
  }

  if (!form.password) {
    errors.password = t('pages.login.passwordRequired'); // 修改为 t()
  }
  ...
  try {
    ...
    window.$notify.success(
      t('pages.login.successTitle'), // 修改为 t()
      t('pages.login.successMessage') // 修改为 t()
    );
  } catch (error) {
    // 显示错误信息
    errorMessage.value = error.response?.data?.message || t('pages.login.errorMessage'); // 修改为 t()
  } finally {
    loading.value = false;
  }
};
