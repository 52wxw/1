      ...
      <div class="container mx-auto">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div class="relative">
            ...
            >
              <option value="all">{{ $t('taskList.allCategories') }}</option>
              <option 
          ...
          <div class="relative">
            <select
              ...
              class="bg-sub-bg border border-gray-700 text-white rounded-lg py-2 pl-4 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"
            >
              <option value="all">{{ $t('taskList.allDifficulties') }}</option>
              <option value="easy">{{ $t('taskList.difficulties.easy') }}</option>
              <option value="medium">{{ $t('taskList.difficulties.medium') }}</option>
              <option value="hard">{{ $t('taskList.difficulties.hard') }}</option>
              <option value="expert">{{ $t('taskList.difficulties.expert') }}</option>
            </select>
            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none">
              <i class="fa-solid fa-chevron-down text-gray-400"></i>
            </div>
...
const fetchTasks = async () => {
  try {
    ...
    window.$notify.error(
      $t('taskList.fetchErrorTitle'),
      $t('taskList.fetchErrorMessage')
    );
  } finally {
    loading.value = false;
  }
...
const handleSolveTask = (taskId) => {
  ...
  window.$notify.success(
    $t('taskList.solveSuccessTitle'),
    $t('taskList.solveSuccessMessage')
  );
